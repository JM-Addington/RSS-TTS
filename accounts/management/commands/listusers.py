...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options.get('pending_only'):
            queryset = queryset.filter(profile__is_approved=False, profile__is_super_admin=False)
    ...
    def output_table(self, queryset, options):
        ...
        self.stdout.write(header_format.format(
            'Username', 'Email', 'Status', 'Type', 'Joined'
        ))
        self.stdout.write('-' * 80)
        ...
        for user in queryset:
            ...
            self.stdout.write(style(header_format.format(
        ...
        # Summary stats
        if not any([options.get('pending_only'), options.get('admins_only'), options.get('approved_only')]):
    ...
    def output_csv(self, queryset):
        ...
        writer.writerow([
            ...
            'is_active', 'is_approved', 'is_super_admin',
    ...
    def _show_summary_stats(self):
        ...
        super_admins = User.objects.filter(profile__is_super_admin=True, is_active=True).count()
        approved_users = User.objects.filter(profile__is_approved=True, is_active=True, profile__is_super_admin=False).count()
        pending_users = User.objects.filter(profile__is_approved=False, profile__is_super_admin=False, is_active=True).count()
        ...
        if pending_users > 0:
            self.stdout.write('')
            self.stdout.write(self.style.WARNING(f'{pending_users} user(s) need approval!'))
