...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if demote:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Successfully demoted user "{username}" from super admin')
            )
            self.stdout.write(
                self.style.WARNING(
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Successfully promoted user "{username}" to super admin')
            )
            self.stdout.write(
                self.style.SUCCESS('User has been automatically approved and granted staff access.')
            )
    ...
    def list_super_admins(self):
        ...
        if not super_admins.exists():
            ...
            self.stdout.write('This should not happen. Create a super admin immediately.')
            return
        ...
        for user in super_admins:
            ...
            self.stdout.write(f'  • {user.username}{email_info} - {approval_status} - joined {joined_date}')
        ...
        self.stdout.write('To demote a user: python manage.py promoteuser <username> --demote')
