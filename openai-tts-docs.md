...
### Prompt Field for Audit and Debugging

The RSS-TTS system now stores computed TTS prompts in the Article model for audit and debugging purposes:

- **Persistent Storage**: The `Article.prompt` field stores the enhanced TTS prompt generated by the voice parameter generation service
- **Auto-Generated Content**: When using AUTO voice mode, the system generates context-aware prompts that are saved to this field
- **Debugging Support**: Administrators can view the exact prompt used for TTS generation via the Django admin interface
- **Audit Trail**: The stored prompts provide a historical record of voice instructions used for each article
- **Read-Only Access**: The prompt field is read-only in the admin interface to preserve the generated content

**Implementation Details:**

- **VoiceConfigurationService**: Automatically populates the prompt field when enhanced prompts are generated
- **Database Field**: TextField with blank=True and default="" to support both populated and empty states
- **Migration Support**: Existing articles will have empty prompts until regenerated with AUTO voice mode

This feature enhances debugging capabilities and provides transparency into the AI-driven voice parameter generation process.
