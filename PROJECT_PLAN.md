...
| **Technology Stack** | Django 5 + DRF • Celery 6 + Redis • SQLite • OpenAI TTS (`tts-1-hd` / `gpt-4o-mini-tts`) • OpenAI o3-mini & GPT-4.1 for tone/extraction • Docker Compose dev/prod • Bootstrap 5 for UI.                                                                 |
...
* **Define Celery Task for Article Processing:** Write a Celery task (e.g., `tasks.process_article(article_id)`) that performs the following sub-tasks in sequence: *(Est.: 2 days total)* ✅
  ...
  3. **Call OpenAI TTS API for Each Chunk:** Use OpenAI's Python SDK to call the text-to-speech endpoint for each chunk. For example: `openai.audio.speech.create(model="tts-1-hd", voice=VOICE, input=chunk)` to get an audio stream. Use an appropriate voice (e.g., "nova" or "alloy" as default) and capture the audio. Stream each response to a temporary MP3 file. Include error handling (e.g., API errors or timeouts). ✅
