...
* **Epic 0.2: Dockerized Development Environment** – *Containerize the app and services for consistent dev and prod environments.*
  ...
  * **Docker Compose Configuration:** Write a `docker-compose.yml` to define services: Django web app, Celery worker, message broker (Redis), and a database (PostgreSQL). Include a Redis service for Celery’s broker (Redis is a common choice for Celery message backend) and Postgres for the Django database. Ensure that running `docker-compose up` will launch all containers for local development. *(Est.: 1 day)*
