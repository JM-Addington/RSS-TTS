"""Mock tasks module for testing without audio dependencies."""
...
# Export MockAudioSegment as AudioSegment for importing code
...
def process_article(self, article_id: int) -> str:
    ...
    try:
        ...
        # Get user_id and feed_id safely
        user_id = (
            str(article.feed.user_id) if hasattr(article.feed, "user_id") else "unknown"
        )
        feed_id = str(article.feed.id) if hasattr(article.feed, "id") else "unknown"
        article_media_dir = media_root / "articles" / user_id / feed_id
        ...
        # For tests, we'll mock the OpenAI API calls
        if settings.TESTING:
            ...
            for i, chunk in enumerate(text_chunks):
                ...
                with open(temp_file_path, "w") as f:
                    f.write(f"Mock audio data for chunk {i}")
                temp_audio_files.append(temp_file_path)
                logger.debug(f"Saved mock audio chunk to {temp_file_path}")
        else:
            client = openai.OpenAI(api_key=settings.OPENAI_API_KEY)

            for i, chunk in enumerate(text_chunks):
                # Format debug message to be under 88 chars
                chunk_info = f"Processing chunk {i+1}/{len(text_chunks)}"
                logger.debug(f"{chunk_info} for Article: {article_id}")
                temp_file_path = (
                    article_media_dir
                    / f"temp_article_{article_id}_chunk_{i}_{uuid.uuid4()}.mp3"
                )
        ...
        if len(temp_audio_files) == 1:
            ...
            # For tests, we'll create a mock combined file
            final_audio_path = article_media_dir / f"article_{article.audio_uuid}.mp3"
            with open(final_audio_path, "w") as f:
                f.write(f"Mock combined audio for article {article_id}")
            logger.info(
