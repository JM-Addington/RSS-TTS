...
def process_article(self, article_id: int) -> str:
    ...
    try:
        ...
        # AIDEV-NOTE: article may be deleted (web/MCP) while TTS runs; abort
        # before export or we'd orphan an MP3 and article.save() below would
        # re-insert the deleted row
        if not Article.objects.filter(pk=article_id).exists():
            logger.info(
                f"Article {article_id} was deleted during processing; "
                f"skipping audio export."
            )
            return f"Article {article_id} deleted during processing; export skipped."
