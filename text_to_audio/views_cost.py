...
from .models import Article, OpenAIUsageStats
...
class UsageDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        current_month_usage = user_usage.filter(
            request_timestamp__gte=current_month_start
        )
        ...
        article_costs = (
            user_usage.filter(article__isnull=False)
        ...
        operation_usage = (
            user_usage.values("operation_type")
            .annotate(
                total_cost=Sum("estimated_cost"),
                total_tokens=Sum("tokens_used"),
                request_count=Count("id"),
        ...
        context.update(
            {
                ...
                "article_costs": [
                    {
                        **item,
                        "total_cost": format_cost_display(
...
class ArticleCostDetailView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        usage_records = OpenAIUsageStats.objects.filter(article=article).order_by(
            "-request_timestamp"
        )
        ...
        operation_breakdown = (
            usage_records.values("operation_type")
            .annotate(
                total_cost=Sum("estimated_cost"),
                total_tokens=Sum("tokens_used"),
                request_count=Count("id"),
        ...
        # Group by model
        model_breakdown = (
            usage_records.values("model_name")
            .annotate(
                total_cost=Sum("estimated_cost"),
                total_tokens=Sum("tokens_used"),
                request_count=Count("id"),
