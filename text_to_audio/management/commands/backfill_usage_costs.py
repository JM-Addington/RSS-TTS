...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        records_without_cost = OpenAIUsageStats.objects.filter(
        ...
        self.stdout.write(f"Found {total_count} usage records without cost estimates.")
        ...
        while processed < total_count:
            with transaction.atomic():
                # Get next batch
                batch = list(records_without_cost[processed : processed + batch_size])
                        ...
                        if not dry_run:
                            ...
                            if record.operation_type == "LLM" and record.tokens_used:
                                estimated_cost = estimate_cost_from_total_tokens(
                                    record.model_name, record.tokens_used
                                )
                                self.stdout.write(
