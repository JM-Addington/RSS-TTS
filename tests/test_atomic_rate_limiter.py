...
class AtomicRateLimiterTests(TestCase):
    ...
    def test_lua_script_atomicity(self):
        ...
        print(
            f"Rate limiter test results: {successful_acquisitions} granted, {denied_requests} denied out of {total_requests} total"
        )
    ...
    def test_lua_script_fallback_on_redis_error(self):
        ...
        with patch("text_to_audio.rate_limiter.redis.Redis") as mock_redis_class:
            ...
            mock_redis_instance.script_load.side_effect = redis.RedisError(
                "Redis connection failed"
            )
            mock_redis_instance.evalsha.side_effect = redis.RedisError(
            ...
            result = rate_limiter._check_and_acquire()
            self.assertTrue(
                result, "Rate limiter should allow requests when Redis fails"
            )
    ...
    def test_lua_script_handles_noscript_error(self):
        ...
        with patch("text_to_audio.rate_limiter.redis.Redis") as mock_redis_class:
            ...
            result = rate_limiter._check_and_acquire()
            self.assertTrue(
                result, "Rate limiter should handle NOSCRIPT error and succeed"
            )
