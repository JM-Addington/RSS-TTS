...
class ProcessArticleTests(TestCase):
    ...
    def test_temp_files_cleaned_up_on_failure(self, MockOpenAIClient):
        ...
        # Skip stats creation to avoid transaction issues
        with patch("text_to_audio.tasks._save_openai_usage_stats"), \
             patch("text_to_audio.tasks.ContentAnalysisService") as MockContentAnalysisService:
    ...
    def test_process_article_successful_multi_voice(
        self, mock_audio_empty, mock_audio_from_mp3, MockContentAnalysisService, MockOpenAIClient
    ):
        ...
        mock_audio_segment = MagicMock()
        mock_audio_segment.set_frame_rate.return_value = mock_audio_segment
        mock_audio_segment.export.side_effect = self.create_dummy_file_side_effect # Simulate file creation
        mock_audio_from_mp3.return_value = mock_audio_segment
        ...
        # Mock _is_valid_multi_voice_data to return True to ensure multi-voice path is taken
    ...
    def test_process_article_multi_voice_segment_chunking(
        ...
        long_segment_text = "This is a very long segment. " * 300 # Approx 7*300 = 2100 chars
        short_segment_text = "This is short."

        multi_voice_data_with_long_segment = {
            "voices": [
                {"name": "long_talker", "tone": "verbose", "tts_model": "fable", "tts_speed": 1.0},
                {"name": "short_talker", "tone": "concise", "tts_model": "shimmer", "tts_speed": 0.8},
            ],
            "audio_segments": [
        ...
        self.article.text_content = long_segment_text + short_segment_text # For analysis sample if it ran
        self.article.save()
        
        # We need to know how many chunks _chunk_text will make for long_segment_text
        # Assuming default max_length for _chunk_text is 4000. Our long_segment_text is ~2100.
        # So, it should be 1 chunk based on length. Let's make it longer to force chunking.
        # Let's assume max_length is small for testing chunking within a segment.
        # The _chunk_text in tasks.py has max_length=4000.
        # So we need to make long_segment_text > 4000 to test chunking.
        long_segment_text_actually_long = "This is an extremely long segment designed to test chunking. " * 250 # > 4000 chars
        
        multi_voice_data_with_long_segment["audio_segments"][0]["text"] = long_segment_text_actually_long
        self.article.multi_voice_data = multi_voice_data_with_long_segment
        self.article.save()

        # Let's spy on _chunk_text to verify its calls
        with patch("text_to_audio.tasks._is_valid_multi_voice_data", return_value=True), \
             patch("text_to_audio.tasks._chunk_text") as mock_internal_chunk_text, \
             patch("text_to_audio.tasks._save_openai_usage_stats") as mock_save_stats:
            
            # Make _chunk_text behave normally for the first call (long segment)
            # and return 1 chunk for the second call (short segment)
            # The first segment (long_segment_text_actually_long) will be chunked by the actual _chunk_text
            # The second segment (short_segment_text) will also be chunked by the actual _chunk_text
            
            # To properly test this, we need to let the actual _chunk_text run.
            # We are interested in the calls to speech.create.
            # If long_segment_text_actually_long results in N chunks, and short_segment_text in 1 chunk,
            # then speech.create should be called N+1 times.
            ...
            # Short segment "This is short." is 1 chunk. Total = 5 calls to speech.create.
            
            # We can't easily mock _chunk_text differently for different calls within the loop
            # So, we'll rely on the actual _chunk_text behavior.
        ...
        for i in range(4):
            self.assertEqual(calls[i][1]["voice"], "fable") # tts_model from "long_talker"
        ...
        self.assertEqual(calls[4][1]["voice"], "shimmer") # tts_model from "short_talker"
...
from text_to_audio.tasks import poll_followed_feeds

@override_settings(MEDIA_ROOT=TEST_MEDIA_ROOT, OPENAI_API_KEY="test_api_key")
class PollFollowedFeedsTaskTests(TestCase):
    """Tests for the poll_followed_feeds Celery task."""

    def setUp(self):
        """Set up test data."""
        self.user = User.objects.create_user(
            username="polltestuser", password="password"
        )
        self.destination_feed = Feed.objects.create(
            user=self.user, name="Poll Destination Feed"
        )
    ...
    def test_basic_feed_polling_new_entries(
        self, mock_feedparser_parse, mock_process_url_to_text, mock_process_article_delay
    ):
        """Test basic feed polling creates articles and enqueues tasks."""
        ...
        mock_feed_data.entries = [
            MagicMock(id="guid1", title="Title 1", link="http://example.com/entry1", summary="Summary 1"),
            MagicMock(id="guid2", title="Title 2", link="http://example.com/entry2", summary="Summary 2"),
        ]
        mock_feedparser_parse.return_value = mock_feed_data

        poll_followed_feeds()

        articles = Article.objects.filter(feed=self.destination_feed).order_by("created_at")
        self.assertEqual(articles.count(), 2)
        self.assertEqual(articles[0].title, "Title 1")
        self.assertEqual(articles[0].text_content, "Summary 1")
        self.assertEqual(articles[1].title, "Title 2")
        self.assertEqual(articles[1].text_content, "Summary 2")

        self.assertEqual(mock_process_article_delay.call_count, 2)
        mock_process_article_delay.assert_has_calls([
            call(articles[0].id),
            call(articles[1].id),
        ])

        followed_feed.refresh_from_db()
        self.assertEqual(followed_feed.last_guid, "guid2") # Newest entry's GUID
        self.assertIsNotNone(followed_feed.last_checked)
        self.assertEqual(mock_process_url_to_text.call_count, 0) # Content was in summary

    @patch("text_to_audio.tasks.process_article.delay")
    @patch("text_to_audio.tasks.feedparser.parse")
    def test_skipping_processed_entries(
        self, mock_feedparser_parse, mock_process_article_delay
    ):
        """Test that entries older than last_guid are skipped."""
        followed_feed = FollowedFeed.objects.create(
            user=self.user,
            url="https://example.com/feed2",
            destination_feed=self.destination_feed,
            is_active=True,
            last_guid="guid_old", # This entry should be considered processed
        )

        mock_feed_data = MagicMock()
        mock_feed_data.bozo = 0
        mock_feed_data.entries = [ # Entries are typically newest first
            MagicMock(id="guid_new", title="New Title", link="http://example.com/new", summary="New Summary"),
            MagicMock(id="guid_old", title="Old Title", link="http://example.com/old", summary="Old Summary"),
        ]
        mock_feedparser_parse.return_value = mock_feed_data

        poll_followed_feeds()

        articles = Article.objects.filter(feed=self.destination_feed)
        self.assertEqual(articles.count(), 1)
        self.assertEqual(articles[0].title, "New Title")

        mock_process_article_delay.assert_called_once_with(articles[0].id)

        followed_feed.refresh_from_db()
        self.assertEqual(followed_feed.last_guid, "guid_new")

    @patch("text_to_audio.tasks.process_article.delay")
    @patch("text_to_audio.tasks.process_url_to_text")
    @patch("text_to_audio.tasks.feedparser.parse")
    def test_content_extraction_fallback(
        self, mock_feedparser_parse, mock_process_url_to_text, mock_process_article_delay
    ):
        """Test fallback to process_url_to_text if content is missing."""
        ...
        mock_feed_data = MagicMock()
        mock_feed_data.bozo = 0
        # Entry with no direct content (summary, content[0].value etc.)
        mock_feed_data.entries = [
            MagicMock(id="guid_fetch", title="Fetch Title", link="http://example.com/fetch", summary=None, content=None)
        ]
        # Remove summary and content attributes if they exist to ensure fallback
        del mock_feed_data.entries[0].summary 
        del mock_feed_data.entries[0].content


        mock_feedparser_parse.return_value = mock_feed_data
        mock_process_url_to_text.return_value = (True, "Fetched text content", None)

        poll_followed_feeds()
        
        articles = Article.objects.filter(feed=self.destination_feed)
        self.assertEqual(articles.count(), 1)
        self.assertEqual(articles[0].title, "Fetch Title")
        self.assertEqual(articles[0].text_content, "Fetched text content")

        mock_process_url_to_text.assert_called_once_with("http://example.com/fetch")
        mock_process_article_delay.assert_called_once_with(articles[0].id)

    @patch("text_to_audio.tasks.logger.error")
    @patch("text_to_audio.tasks.feedparser.parse")
    def test_feedparser_error_handling(self, mock_feedparser_parse, mock_logger_error):
        """Test graceful handling of feedparser errors."""
        FollowedFeed.objects.create(
            user=self.user,
            url="https://example.com/badfeed",
            destination_feed=self.destination_feed,
            is_active=True,
        )
        mock_feedparser_parse.side_effect = Exception("Feedparser boom!")

        poll_followed_feeds() # Should not raise an exception

        self.assertEqual(Article.objects.count(), 0) # No articles created
        mock_logger_error.assert_called_with(
            "Error parsing feed https://example.com/badfeed: Feedparser boom!"
        )

    @patch("text_to_audio.tasks.feedparser.parse")
    def test_inactive_feeds_skipped(self, mock_feedparser_parse):
        """Test that inactive feeds are skipped."""
        FollowedFeed.objects.create(
            user=self.user,
            url="https://example.com/inactive_feed",
            destination_feed=self.destination_feed,
    ...
    @patch("text_to_audio.tasks.process_article.delay")
    @patch("text_to_audio.tasks.feedparser.parse")
    def test_bozo_feed_still_processed(
        self, mock_feedparser_parse, mock_process_article_delay
    ):
        """Test that feeds with bozo bit set are still processed."""
        followed_feed = FollowedFeed.objects.create(
            user=self.user,
            url="https://example.com/bozofeed",
            destination_feed=self.destination_feed,
            is_active=True,
        )

        mock_feed_data = MagicMock()
        mock_feed_data.bozo = 1 # Indicates a problem, but often parsable
        mock_feed_data.bozo_exception = "Some minor issue"
        mock_feed_data.entries = [
            MagicMock(id="guid_bozo", title="Bozo Title", link="http://example.com/bozo1", summary="Bozo Summary"),
        ]
        mock_feedparser_parse.return_value = mock_feed_data
        ...
        self.assertTrue(any("Feed https://example.com/bozofeed may be ill-formed." in message for message in log_watcher.output))

        articles = Article.objects.filter(feed=self.destination_feed)
        self.assertEqual(articles.count(), 1)
        self.assertEqual(articles[0].title, "Bozo Title")
        mock_process_article_delay.assert_called_once_with(articles[0].id)
        followed_feed.refresh_from_db()
        self.assertEqual(followed_feed.last_guid, "guid_bozo")

    @patch("text_to_audio.tasks.process_article.delay")
    @patch("text_to_audio.tasks.feedparser.parse")
    def test_no_entries_in_feed(
        self, mock_feedparser_parse, mock_process_article_delay
    ):
        """Test handling of feeds that return no entries."""
        followed_feed = FollowedFeed.objects.create(
            user=self.user,
            url="https://example.com/emptyfeed",
            destination_feed=self.destination_feed,
            is_active=True,
        )
        
        initial_last_checked = followed_feed.last_checked

        mock_feed_data = MagicMock()
        mock_feed_data.bozo = 0
        mock_feed_data.entries = [] # No entries
        mock_feedparser_parse.return_value = mock_feed_data
        
        with self.assertLogs("text_to_audio.tasks", level="INFO") as log_watcher:
            poll_followed_feeds()

        self.assertTrue(any("No entries found in feed: https://example.com/emptyfeed" in message for message in log_watcher.output))
        self.assertEqual(Article.objects.count(), 0)
        mock_process_article_delay.assert_not_called()
        
        followed_feed.refresh_from_db()
        self.assertIsNotNone(followed_feed.last_checked)
        if initial_last_checked: # Ensure it was updated
            self.assertTrue(followed_feed.last_checked > initial_last_checked)
        else: # Ensure it was set
             self.assertTrue(followed_feed.last_checked is not None)

    @patch("text_to_audio.tasks.process_article.delay")
    @patch("text_to_audio.tasks.feedparser.parse")
    def test_last_guid_not_found_in_feed(
        self, mock_feedparser_parse, mock_process_article_delay
    ):
        """Test behavior when last_guid is not found in the current feed entries (processes all)."""
        followed_feed = FollowedFeed.objects.create(
            user=self.user,
            url="https://example.com/feed_guid_vanished",
            destination_feed=self.destination_feed,
            is_active=True,
            last_guid="guid_that_is_gone",
        )

        mock_feed_data = MagicMock()
        mock_feed_data.bozo = 0
        mock_feed_data.entries = [
            MagicMock(id="new_guid1", title="New Title 1", link="http://example.com/new1", summary="Summary 1"),
            MagicMock(id="new_guid2", title="New Title 2", link="http://example.com/new2", summary="Summary 2"),
        ] # last_guid="guid_that_is_gone" is not in this list
        mock_feedparser_parse.return_value = mock_feed_data
        
        with self.assertLogs("text_to_audio.tasks", level="WARNING") as log_watcher:
            poll_followed_feeds()

        self.assertTrue(any("Last processed GUID guid_that_is_gone not found" in message for message in log_watcher.output))
        
        articles = Article.objects.filter(feed=self.destination_feed).order_by("created_at")
        self.assertEqual(articles.count(), 2) # Both entries should be processed
        self.assertEqual(mock_process_article_delay.call_count, 2)

        followed_feed.refresh_from_db()
        self.assertEqual(followed_feed.last_guid, "new_guid1") # GUID of the newest entry
        self.assertIsNotNone(followed_feed.last_checked)
