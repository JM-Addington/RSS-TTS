...
class SpeedControlChunkToneTests(TestCase):
    ...
    def test_speed_control_propagated_to_chunk_tone_service(
        ...
        mock_payload_125 = ChunkTonePayload(
            chunks=[
                ...
                ChunkData(text="for 1.25x speed article.", voice=TTSVoice(voice="echo")),
            ]
        )
        ...
        with patch("text_to_audio.tasks.ContentAnalysisService") as MockContentAnalysisService, \
        ...
        self.assertEqual(result_125, f"Article {article_125.id} processed successfully.")
        self.assertEqual(result_100, f"Article {article_100.id} processed successfully.")
    ...
    def test_speed_from_voice_parameters_prioritized(
        ...
        mock_payload = ChunkTonePayload(
            chunks=[
                ChunkData(text="This is test content", voice=TTSVoice(voice="alloy")),
                ChunkData(text="for voice_parameters speed test.", voice=TTSVoice(voice="echo")),
            ]
        )
    ...
    def test_speed_fallback_when_none_specified(
        ...
        mock_payload = ChunkTonePayload(
            chunks=[
                ChunkData(text="This is test content", voice=TTSVoice(voice="alloy")),
                ChunkData(text="with no speed specified.", voice=TTSVoice(voice="echo")),
            ]
        )
        mock_chunk_tone_service.get_payload.return_value = mock_payload
