...
class TestContentAnalysisService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.service = ContentAnalysisService(openai_api_key="fake_key_override") # Test with override
    ...
    def test_successful_summary_extraction(self):
        ...
        expected_voices = [{"name": "narrator", "tone": "neutral", "tts_model": "alloy", "tts_speed": 1.0}]
    ...
    def test_invalid_json_response_from_llm(self):
        ...
        self.assertEqual(result.get("audio_segments")[0]["text"], sample_text) # Original full text
        self.assertEqual(result.get("audio_segments")[0]["voice_name"], "narrator")
        self.mock_openai_client.chat.completions.create.assert_called_once()

    def test_api_error_propagated(self):
        self.mock_openai_client.chat.completions.create.side_effect = openai.APIError("Test API Error", request=None, body=None)
    ...
    def test_empty_voices_or_segments_in_llm_response(self):
        ...
        self.assertEqual(result1.get("audio_segments")[0]["text"], sample_text) # Fallback uses original text
        ...
        self.assertEqual(result2.get("audio_segments")[0]["text"], sample_text_2) # Fallback uses original text
