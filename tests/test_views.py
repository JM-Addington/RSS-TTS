...
class RegenerateArticleViewTest(TestCase):
    ...
    def test_regenerate_article(self, mock_process_article):
        """Test that regenerating an article creates a new article and queues processing."""
        # Get the initial article count
        initial_count = Article.objects.count()

        # Make the post request to regenerate
        response = self.client.post(
            reverse("article-regenerate", kwargs={"article_id": self.article.id})
        )
        ...
        # Get the new article
        new_article = Article.objects.exclude(id=self.article.id).first()
        ...
        mock_process_article.assert_called_once_with(new_article.id)
    ...
    def test_regenerate_article_access_control(self):
        ...
        response = self.client.post(
            reverse("article-regenerate", kwargs={"article_id": other_article.id})
        )
